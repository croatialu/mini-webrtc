...
import type { WSAnnouncePayload, WSPublishPayload, WSSignalPayload, WSSubscribePayload } from './types'
...
function createSignalPayload(
  roomName: string,
  data: Omit<WSSignalPayload, 'type'>,
): WSPublishPayload<WSSignalPayload> {
  return {
    type: 'publish',
    topic: roomName,
    data: {
...
export class SignalingConn extends Observable<SignalingConnEvents> {
  static New = (url: string, webSocketConstructor?: WebSocket) => {
    const signalingConn = signalingConns.get(url) || new SignalingConn(url, webSocketConstructor)
  ...
  constructor(
    ...
    const ws = this.ws = webSocketConstructor ? new (webSocketConstructor as any)(url) : new WebSocket(url)
